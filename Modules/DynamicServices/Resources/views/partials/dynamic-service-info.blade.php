            ...
            <div class="d-flex align-items-center mb-3">
                <span class="avatar avatar-md me-2 bg-primary-gradient">
                    @if ($dynamicService->getFirstMedia('icon'))
                        <img src="{{ $dynamicService->getFirstMedia('icon')->getUrl() }}"
                            alt="{{ $dynamicService->title }}" class="card-img">
                    @else
                                    ...
                                    @switch($field['type'] ?? 'text')
                                        ...
                                        @case('checkbox')
                                            <span class="badge bg-primary-transparent">
                                                <i
                                                    class="ri-checkbox-line me-1"></i>{{ \Modules\DynamicServices\Helpers\DynamicServiceViewHelper::getFieldDisplayValue($field) }}
                                            </span>
                                        @break
                                        ...
                                        @case('file')
                                            @if (!empty($field['value']))
                                                <a href="{{ $field['value'] }}" target="_blank"
                                                    ...
                                                    {{ $field['original_name'] ?? ($field['file_name'] ?? trans('global.download')) }}
                                                </a>
                                            @else
