(function () {
  ...
  // Set to true to persist theme in localStorage; false = DB only (no theme keys in localStorage)
  const THEME_SAVE_TO_LOCAL_STORAGE = false;
  const themeSetItem = (key, value) => { if (THEME_SAVE_TO_LOCAL_STORAGE) localStorage.setItem(key, value); };
...
if (document.querySelector("#switcher-canvas")) {
    ...
    for (const [theme, config] of nanoThemes) {
      ...
      button.addEventListener("click", () => {
        ...
        nanoPickr.on("changestop", (source, instance) => {
          ...
          const rgb = `${Math.floor(color[0])}, ${Math.floor(color[1])}, ${Math.floor(color[2])}`;
          let html = document.querySelector("html");
          html.style.setProperty("--primary-rgb", rgb);
          themeSetItem("primaryRGB", rgb);
          document.dispatchEvent(new CustomEvent("theme-primary-changed", { detail: { value: rgb } }));
        });
      });
    ...
    for (const [theme, config] of nanoThemes) {
      ...
      button.addEventListener("click", () => {
        ...
        nanoPickr1.on("changestop", (source, instance) => {
          let color = instance.getColor().toRGBA();
          const bg = `${Math.floor(color[0])}, ${Math.floor(color[1])}, ${Math.floor(color[2])}`;
          const light = `${Math.floor(color[0]) + 14}, ${Math.floor(color[1]) + 14}, ${Math.floor(color[2]) + 14}`;
          let html = document.querySelector("html");
          html.style.setProperty("--body-bg-rgb", bg);
          html.style.setProperty("--body-bg-rgb2", light);
          html.style.setProperty("--light-rgb", light);
          html.style.setProperty("--form-control-bg", "rgb(" + light + ")");
          html.style.setProperty("--input-border", "rgba(255,255,255,0.1)");
          html.style.setProperty("--gray-3", "rgba(255,255,255,0.1)");
          themeSetItem("bodyBgRGB", bg);
          themeSetItem("bodylightRGB", light);
          ...
          document.dispatchEvent(new CustomEvent("theme-background-changed", { detail: { bg, light } }));
        });
      });
  ...
  /* header theme toggle: same behavior as switcher Light/Dark radios */
  function toggleTheme() {
    ...
    /* fallback when switcher is not in DOM */
    let html = document.querySelector("html");
    if (html.getAttribute("data-theme-mode") === "dark") {
      ...
      themeSetItem("zenodarktheme", "true");
